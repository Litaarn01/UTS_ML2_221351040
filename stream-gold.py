...
if st.button('Prediksi'):
    ...
    if predicted_price is not None:
        st.success(f"Harga emas pada {future_date} diperkirakan: {format_rupiah(predicted_price)}")
    else:
...
if st.button('Lihat Naik dan Turun Harga Emas'):
    ...
    if highest_price is not None and lowest_price is not None:
        ...
        st.write(f"Harga Tertinggi: {format_rupiah(highest_price)}")
        st.write(f"Harga Terendah: {format_rupiah(lowest_price)}")
